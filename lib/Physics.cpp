...
Vec3f refract(const Vec3f& in, const Vec3f& normal, const float refractive_coefficient)
{
    ...
    float refractive_index_outer = 1, refractive_index_inner = refractive_coefficient;//默认从外部射向内部
    Vec3f n = normal;
    ...
    return k > 0 ? in * refractive_ratio + n * (refractive_ratio * cosi - sqrt(k)) : reflect(in, normal);
}