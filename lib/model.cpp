...
bool Model::bbox_intersect(const Vec3f& orig, const Vec3f& dir)
{
    Vec3f t_enter((min.x - orig.x) / dir.x, (min.y - orig.y) / dir.y, (min.z - orig.z) / dir.z);
    Vec3f t_exit((max.x - orig.x) / dir.x, (max.y - orig.y) / dir.y, (max.z - orig.z) / dir.z);
    for (size_t i = 0; i < 3; i++)
    {
        if (dir[i] < 0) std::swap(t_enter[i], t_exit[i]);
    }
    float max_enter = std::max(t_enter.x, std::max(t_enter.y, t_enter.z));
    float min_exit = std::min(t_exit.x, std::min(t_exit.y, t_exit.z));
    return max_enter < min_exit && min_exit>0 && max_enter > 0;
}
...
Vec3f Model::get_face_normal(int i)
{
